...
while True:
    ...
    directory=topic
    f7 = open("./server2/log.log",'a')
    mes = "Message Sent to Broker 2 --The Topic is :: " + str(topic) + " --The Message is :: " + str(message)
    f7.write(mes+'\n')
    
    f7.close()
    isExist=os.path.exists("./server2/"+directory)
    if not isExist:
        parent_dir="./server2"
        path=os.path.join(parent_dir,directory)
        os.mkdir(path)
        parent_dir="./server2/"+topic
        ...
        count_path="./server2/"+topic+"/partition1"
        count+= len(fnmatch.filter(os.listdir(count_path), '*.txt'))

        count_path="./server2/"+topic+"/partition2"
        count+= len(fnmatch.filter(os.listdir(count_path), '*.txt'))


        count_path="./server2/"+topic+"/partition3"
        count+= len(fnmatch.filter(os.listdir(count_path), '*.txt'))
        ...
        if((count)%3==0):
            ...
            path="./server2/"+topic+"/partition1/"+file_name
            f = open(path, "a")
            f.write(message+"\n")
            f.close()
        if((count)%3==1):
            file_name = str(count+1)+".txt"
            path="./server2/"+topic+"/partition2/"+file_name
            f = open(path, "a")
            f.write(message+"\n")
            f.close()
        if((count)%3==2):
            file_name = str(count+1)+".txt"
            path="./server2/"+topic+"/partition3/"+file_name
            f = open(path, "a")
            f.write(message+"\n")
            f.close()
    else :
        ...
        if((count)%3==2):
            ...
            f.close()
    print(count)       
    source_folder = './server2'
    destination_folder = './server1'
